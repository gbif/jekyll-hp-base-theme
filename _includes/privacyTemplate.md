...
For example, we ask visitors who send a message to our Help Desk, requesting assistance, to provide an email address. {{ site.privacy.name }} collects such information only insofar as is necessary or appropriate to fulfil the purpose of the visitor’s interaction with {{ site.privacy.name }}. {{ site.privacy.name }} does not disclose personal data other than as described in the Protection of Information section below. Visitors can always refuse to supply personal data, with the caveat that it may prevent them from engaging in certain activities.
